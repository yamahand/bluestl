...
namespace bluestl
{
	...
	struct pair
	{
		...
		// コピーコンストラクタ
		constexpr pair(const pair &p) noexcept(std::is_nothrow_copy_constructible_v<T1> && std::is_nothrow_copy_constructible_v<T2>)
			requires std::is_copy_constructible_v<T1> && std::is_copy_constructible_v<T2>
		= default;
		// ムーブコンストラクタ
		constexpr pair(pair &&p) noexcept(std::is_nothrow_move_constructible_v<T1> && std::is_nothrow_move_constructible_v<T2>)
			requires std::is_move_constructible_v<T1> && std::is_move_constructible_v<T2>
		= default;
		// コピー代入
		constexpr pair &operator=(const pair &p) noexcept(std::is_nothrow_copy_assignable_v<T1> && std::is_nothrow_copy_assignable_v<T2>)
			requires std::is_copy_assignable_v<T1> && std::is_copy_assignable_v<T2>
		= default;
		// ムーブ代入
		constexpr pair &operator=(pair &&p) noexcept(std::is_nothrow_move_assignable_v<T1> && std::is_nothrow_move_assignable_v<T2>)
			requires std::is_move_assignable_v<T1> && std::is_move_assignable_v<T2>
		= default;

		// 通常のコンストラクタ
		constexpr pair(const T1 &f, const T2 &s) noexcept(std::is_nothrow_copy_constructible_v<T1> && std::is_nothrow_copy_constructible_v<T2>)
			: first(f), second(s)
		{
		}
		constexpr pair(T1 &&f, T2 &&s) noexcept(std::is_nothrow_move_constructible_v<T1> && std::is_nothrow_move_constructible_v<T2>)
		...
		constexpr pair(Tuple1 &&t1, Tuple2 &&t2)
			: first(std::get<0>(std::forward<Tuple1>(t1))),
			  second(std::get<0>(std::forward<Tuple2>(t2)))
		{
		}
...
#if _HAS_CXX20
		// <=>（宇宙船演算子）による比較
		friend constexpr auto operator<=>(const pair &lhs, const pair &rhs) noexcept(noexcept(lhs.first <=> rhs.first) && noexcept(lhs.second <=> rhs.second))
			requires requires { lhs.first <=> rhs.first; lhs.second <=> rhs.second; }
		{
